...
import { MarketData, LlamaProtocolResponse, LlamaYieldsResponse, Market, Vault, Token, RiskMetrics } from "./types";

async function fetchDeFiData(): Promise<MarketData[]>  {
    // Helper function to fetch data
     async function fetchAPI<T>(endpoint: string): Promise<T> {
    const response = await fetch(`https://pro-api.llama.fi/${process.env.DEFI_LLAMA_API_KEY}${endpoint}`);
    if (!response.ok) {
...
try {
    ...
    moonwellData.pools?.forEach((pool: any) => {
      if (pool.token === tokenAddresses.WETH || pool.token === tokenAddresses.USDC) {
        moonwellMarkets[pool.pool] = {
          ...
          liquidity: BigInt(Math.floor((pool.tvlUsd - (pool.borrowUsd || 0)) * 1e6)),
    ...
    morphoData.pools?.forEach((pool: { token: string; apyBase: any; tvlUsd: number; }) => {
      if (pool.token === tokenAddresses.WETH || pool.token === tokenAddresses.USDC) {
    ...
    Object.values(tokenAddresses).forEach(address => {
      const tokenData = yieldsData.find((p: { token: string; }) => p.token === address);
      tokens[address] = {
    ...
    [...Object.keys(moonwellMarkets), ...Object.keys(morphoVaults)].forEach(address => {
      const protocolData = moonwellData.pools?.find((p: { pool: string; }) => p.pool === address) || 
